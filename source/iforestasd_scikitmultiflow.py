...
self.n_estimators
    ...
    def __init__(self, window_size=100, n_estimators=25, anomaly_threshold=0.5,
        ...
        self.model_update_windows = []  # To count the number of times the model have been updated 0 Not updated and
        # 1 updated
            ...
            if self.version == "NDKSWIN":
                ...
                if self.first_time_fit:  # It is the first window
                    ...
                    if self.ndkswin.detected_change():
                        ...
                        self.model_update_windows.append(self.samples_seen)
                        self.update_model(
                            self.prec_window)  # This function will discard completly the old model and create a new one
                    else:
                        self.model_update.append(0)
                        self.model_update_windows.append(self.samples_seen)
            else:
    ...
    """
        ...
        if self.updated_randomly:
            old_trees_idx = random.sample(list(range(len(self.ensemble.trees))),
  ...
  '''
    ...
    """
        ...
        prediction = self.ensemble.predict_from_instances_scores(self.ensemble.anomaly_score(X),
                                                                 self.anomaly_threshold)  # return prediction of all
        # instances
    ...
    """
        ...
        prediction = self.ensemble.predict_from_anomaly_scores(self.predict_proba(self.prec_window),
    ...
    """
        if self.samples_seen <= self.window_size:
            return [-1]
        return self.ensemble.anomaly_score(self.prec_window)[
